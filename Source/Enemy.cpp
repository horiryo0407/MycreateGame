...
void Enemy::Update()
{
	...
	const int BODY_HALF_X = 24;

	// ▼ 上方向チェック（天井）
	if (velocityY < 0)
	{
		if (st->IsBlock(position.x - BODY_HALF_X, position.y - BODY_HALF_Y) ||
			st->IsBlock(position.x + BODY_HALF_X, position.y - BODY_HALF_Y))
		{
			velocityY = 0;

			int tileY = (int)((position.y - BODY_HALF_Y) / 64);
			position.y = (tileY + 1) * 64 + BODY_HALF_Y;
		}
	}
	// ▼ 下方向チェック（床）
	else
